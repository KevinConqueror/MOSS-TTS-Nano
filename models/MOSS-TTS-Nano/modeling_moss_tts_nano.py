...
class MossTTSNanoForCausalLM(MossTTSNanoPreTrainedModel):
    ...
    def _generate_audio_token_ids_with_fallback(
        ...
        *,
        ...
        max_new_frames: int,
        do_sample: bool,
        text_do_sample: Optional[bool],
        audio_do_sample: Optional[bool],
        ...
        adaptive_antidegeneration_enabled: bool,
        adaptive_window_frames: int,
        adaptive_repeat_frame_ratio_threshold: float,
        adaptive_consecutive_repeat_frames_threshold: int,
        adaptive_audio_temperature: float,
        adaptive_audio_top_p: float,
        adaptive_audio_top_k: int,
        adaptive_audio_repetition_penalty: float,
        adaptive_force_stop_consecutive_repeat_frames: int,
        repetition_aware_logit_shaping_enabled: bool,
        repetition_aware_window_tokens: int,
        repetition_aware_frequency_penalty: float,
        repetition_aware_consecutive_penalty: float,
        entropy_aware_decoding_enabled: bool,
        entropy_aware_low_entropy: float,
        entropy_aware_high_entropy: float,
        entropy_aware_temperature: float,
        entropy_aware_top_p: float,
        entropy_aware_top_k: int,
        entropy_aware_hysteresis_enabled: bool,
        entropy_aware_enter_high_steps: int,
        entropy_aware_exit_low_steps: int,
    ...
    def _sample_next_token(
        ...
        repetition_aware_logit_shaping_enabled: bool = False,
        repetition_aware_window_tokens: int = 32,
        repetition_aware_frequency_penalty: float = 0.15,
        repetition_aware_consecutive_penalty: float = 0.5,
        entropy_aware_decoding_enabled: bool = False,
        entropy_aware_low_entropy: float = 0.35,
        entropy_aware_high_entropy: float = 0.65,
        entropy_aware_temperature: float = 0.65,
        entropy_aware_top_p: float = 0.8,
        entropy_aware_top_k: int = 10,
    ) -> torch.LongTensor:
        _, probs = self._prepare_sampling_distribution(
            logits=logits,
            do_sample=do_sample,
    ...
    def _prepare_sampling_distribution(
        ...
        scores = self._apply_repetition_aware_logit_shaping(
            scores=scores,
            previous_token_ids=previous_token_ids,
            enabled=repetition_aware_logit_shaping_enabled,
            window_tokens=repetition_aware_window_tokens,
            frequency_penalty=repetition_aware_frequency_penalty,
            consecutive_penalty=repetition_aware_consecutive_penalty,
        )
        if not do_sample:
        ...
        if entropy_aware_decoding_enabled:
            effective_temperature, effective_top_p, effective_top_k = self._resolve_entropy_aware_sampling_params(
                scores=scores,
                base_temperature=temperature,
                base_top_p=top_p,
                base_top_k=top_k,
                low_entropy=entropy_aware_low_entropy,
                high_entropy=entropy_aware_high_entropy,
                target_temperature=entropy_aware_temperature,
                target_top_p=entropy_aware_top_p,
                target_top_k=entropy_aware_top_k,
            )

        scores = scores / effective_temperature
        if effective_top_k is not None and effective_top_k > 0:
            effective_top_k = min(effective_top_k, scores.shape[-1])
            threshold = torch.topk(scores, effective_top_k, dim=-1).values[..., -1, None]
            scores = scores.masked_fill(scores < threshold, float("-inf"))

        if effective_top_p is not None and 0.0 < effective_top_p < 1.0:
            sorted_scores, sorted_indices = torch.sort(scores, descending=True, dim=-1)
            sorted_probs = torch.softmax(sorted_scores, dim=-1)
            sorted_cumsum = torch.cumsum(sorted_probs, dim=-1)
            sorted_remove = sorted_cumsum > effective_top_p
    ...
    def _apply_repetition_aware_logit_shaping(
        self,
        ...
        if not enabled or previous_token_ids is None:
            return scores
        if window_tokens <= 0 or (frequency_penalty <= 0 and consecutive_penalty <= 0):
            return scores

        token_ids = torch.as_tensor(previous_token_ids, device=scores.device, dtype=torch.long)
        if token_ids.ndim == 1:
        ...
        recent_token_ids = token_ids[:, -int(window_tokens) :]
        shaped_scores = scores.clone()
        vocab_size = shaped_scores.shape[-1]
        for batch_index in range(shaped_scores.shape[0]):
            valid_token_ids = recent_token_ids[batch_index]
            valid_token_ids = valid_token_ids[(valid_token_ids >= 0) & (valid_token_ids < vocab_size)]
            if valid_token_ids.numel() == 0:
                continue

            if frequency_penalty > 0:
                unique_token_ids, counts = torch.unique(valid_token_ids, return_counts=True)
                penalties = counts.to(dtype=shaped_scores.dtype) * float(frequency_penalty)
                token_scores = shaped_scores[batch_index].index_select(0, unique_token_ids) - penalties
                shaped_scores[batch_index].scatter_(0, unique_token_ids, token_scores)

            if consecutive_penalty > 0:
                last_token_id = int(valid_token_ids[-1].item())
                tail_repeats = 1
                for token_id in reversed(valid_token_ids[:-1].tolist()):
                    if int(token_id) != last_token_id:
                        break
                    tail_repeats += 1
                shaped_scores[batch_index, last_token_id] -= float(consecutive_penalty) * float(tail_repeats)
        return shaped_scores

    @staticmethod
    def _resolve_entropy_aware_sampling_params(
        scores: torch.FloatTensor,
        base_temperature: float,
        base_top_p: Optional[float],
        base_top_k: Optional[int],
        low_entropy: float,
        high_entropy: float,
        ...
        entropy_value = float(normalized_entropy.mean().item())

        if high_entropy <= low_entropy:
            alpha = 1.0 if entropy_value >= high_entropy else 0.0
        elif entropy_value <= low_entropy:
            alpha = 0.0
        elif entropy_value >= high_entropy:
            alpha = 1.0
        else:
            alpha = (entropy_value - low_entropy) / (high_entropy - low_entropy)

        effective_temperature = float(base_temperature) + (float(target_temperature) - float(base_temperature)) * alpha
        effective_top_p: Optional[float]
        if base_top_p is None:
            effective_top_p = target_top_p if alpha > 0 else None
        else:
            effective_top_p = float(base_top_p) + (float(target_top_p) - float(base_top_p)) * alpha

        effective_top_k: Optional[int]
        if base_top_k is None:
            effective_top_k = int(target_top_k) if alpha > 0 else None
        else:
            interpolated_top_k = float(base_top_k) + (float(target_top_k) - float(base_top_k)) * alpha
            effective_top_k = max(1, int(round(interpolated_top_k)))
        return effective_temperature, effective_top_p, effective_top_k

    @staticmethod
    def _normalized_distribution_entropy(probs: torch.FloatTensor) -> torch.FloatTensor:
        entropy = -(probs * torch.log(probs.clamp_min(1e-12))).sum(dim=-1)
        return entropy / math.log(probs.shape[-1])

    @staticmethod
    def _update_entropy_aware_state(
        *,
        entropy_values: torch.FloatTensor,
        active_state: torch.BoolTensor,
        high_counts: torch.LongTensor,
        low_counts: torch.LongTensor,
        low_entropy: float,
        high_entropy: float,
        enter_high_steps: int,
        exit_low_steps: int,
    ) -> tuple[torch.BoolTensor, torch.LongTensor, torch.LongTensor]:
        next_active_state = active_state.clone()
        next_high_counts = high_counts.clone()
        next_low_counts = low_counts.clone()
        for batch_index in range(entropy_values.shape[0]):
            entropy_value = float(entropy_values[batch_index].item())
            if entropy_value >= float(high_entropy):
                next_high_counts[batch_index] += 1
                next_low_counts[batch_index] = 0
            elif entropy_value <= float(low_entropy):
                next_low_counts[batch_index] += 1
                next_high_counts[batch_index] = 0
            else:
                next_high_counts[batch_index] = 0
                next_low_counts[batch_index] = 0

            if not bool(next_active_state[batch_index].item()):
                if int(next_high_counts[batch_index].item()) >= max(1, int(enter_high_steps)):
                    next_active_state[batch_index] = True
                    next_high_counts[batch_index] = 0
                    next_low_counts[batch_index] = 0
            else:
                if int(next_low_counts[batch_index].item()) >= max(1, int(exit_low_steps)):
                    next_active_state[batch_index] = False
                    next_high_counts[batch_index] = 0
                    next_low_counts[batch_index] = 0
        return next_active_state, next_high_counts, next_low_counts

    def _select_entropy_triggered_branch_frame(
        self,
        *,
        local_inputs_embeds: torch.FloatTensor,
        current_local_input: torch.FloatTensor,
        effective_nq: int,
        active_generated_audio_history: Optional[torch.LongTensor],
        do_sample: bool,
        audio_temperature: float,
        audio_top_p: float,
        audio_top_k: int,
        ...
        threshold: float,
        num_candidates: int,
        repeat_frame_penalty: float,
        local_dtype: torch.dtype,
    ) -> Optional[torch.LongTensor]:
        if local_inputs_embeds.shape[0] != 1 or effective_nq <= 0:
            return None

        first_history = None if active_generated_audio_history is None else active_generated_audio_history[:, :, 0]
        first_local_inputs = torch.cat([local_inputs_embeds, current_local_input.unsqueeze(1)], dim=1)
        first_local_hidden = self._decode_local_last_hidden_state(first_local_inputs)
        first_channel_logits = self.audio_lm_heads[0](first_local_hidden)
        self._ensure_finite_generation_logits(first_channel_logits, "audio logits[0]")
        _, first_channel_probs = self._prepare_sampling_distribution(
            logits=first_channel_logits,
            do_sample=do_sample,
            temperature=audio_temperature,
            top_k=audio_top_k,
            top_p=audio_top_p,
            previous_token_ids=first_history,
        ...
        first_channel_entropy = float(self._normalized_distribution_entropy(first_channel_probs)[0].item())
        if first_channel_entropy < float(threshold):
            return None

        candidate_count = max(1, min(int(num_candidates), first_channel_probs.shape[-1]))
        candidate_token_ids = torch.topk(first_channel_probs[0], k=candidate_count).indices
        best_frame: Optional[torch.LongTensor] = None
        best_score: Optional[float] = None
        last_frame = None if active_generated_audio_history is None else active_generated_audio_history[:, -1, :]

        for candidate_token_id in candidate_token_ids.tolist():
            branch_local_inputs = local_inputs_embeds
            branch_local_input = current_local_input
            branch_frame_tokens: list[torch.LongTensor] = []
            branch_score = 0.0
            for channel_index in range(effective_nq):
                branch_local_inputs = torch.cat([branch_local_inputs, branch_local_input.unsqueeze(1)], dim=1)
                branch_local_hidden = self._decode_local_last_hidden_state(branch_local_inputs)
                channel_logits = self.audio_lm_heads[channel_index](branch_local_hidden)
                self._ensure_finite_generation_logits(channel_logits, f"audio logits[{channel_index}]")
                previous_channel_ids = (
                    None
                    if active_generated_audio_history is None
                    else active_generated_audio_history[:, :, channel_index]
                )
                _, channel_probs = self._prepare_sampling_distribution(
                    logits=channel_logits,
                    do_sample=do_sample,
                    temperature=audio_temperature,
                    top_k=audio_top_k,
                    top_p=audio_top_p,
                    previous_token_ids=previous_channel_ids,
                    repetition_penalty=audio_repetition_penalty,
                    repetition_aware_logit_shaping_enabled=repetition_aware_logit_shaping_enabled,
                    repetition_aware_window_tokens=repetition_aware_window_tokens,
                    repetition_aware_frequency_penalty=repetition_aware_frequency_penalty,
                ...
                if channel_index == 0:
                    chosen_token = torch.tensor([candidate_token_id], dtype=torch.long, device=channel_probs.device)
                else:
                    chosen_token = channel_probs.argmax(dim=-1)
                selected_prob = channel_probs.gather(1, chosen_token.unsqueeze(-1)).clamp_min(1e-12)
                branch_score += float(torch.log(selected_prob).sum().item())
                branch_frame_tokens.append(chosen_token)
                branch_local_input = self.audio_embeddings[channel_index](chosen_token).to(dtype=local_dtype)

            candidate_frame = torch.stack(branch_frame_tokens, dim=-1)
            if last_frame is not None and candidate_frame.eq(last_frame).all(dim=-1).any():
                branch_score -= float(repeat_frame_penalty)
    ...
    @staticmethod
    def _compute_recent_frame_repeat_stats(
        frame_history: Optional[torch.LongTensor],
        window_frames: int,
    ) -> tuple[torch.FloatTensor, torch.LongTensor]:
        if frame_history is None or frame_history.shape[1] <= 1:
            batch_size = 0 if frame_history is None else frame_history.shape[0]
            device = None if frame_history is None else frame_history.device
            return (
                torch.zeros(batch_size, dtype=torch.float32, device=device),
                torch.zeros(batch_size, dtype=torch.long, device=device),
            )

        recent_history = frame_history[:, -max(2, int(window_frames)) :, :]
        repeated_transitions = recent_history[:, 1:, :].eq(recent_history[:, :-1, :]).all(dim=-1)
        repeat_ratio = repeated_transitions.to(dtype=torch.float32).mean(dim=-1)
        consecutive_repeat_frames = torch.ones(
            frame_history.shape[0],
            dtype=torch.long,
            device=frame_history.device,
        )
        for batch_index in range(frame_history.shape[0]):
            tail_count = 1
            sample_repeats = repeated_transitions[batch_index]
            for repeat_flag in reversed(sample_repeats.tolist()):
                if not repeat_flag:
                    break
                tail_count += 1
            consecutive_repeat_frames[batch_index] = tail_count
        return repeat_ratio, consecutive_repeat_frames

    @staticmethod
    def _merge_rowwise_sampled_tokens(
        base_tokens: torch.LongTensor,
        override_tokens: torch.LongTensor,
        override_mask: torch.BoolTensor,
    ) -> torch.LongTensor:
        if not override_mask.any():
            return base_tokens
        merged = base_tokens.clone()
        merged[override_mask] = override_tokens[override_mask]
        return merged
    ...
    def _iter_generation_events(
        ...
        audio_do_sample: Optional[bool] = None,
        ...
        adaptive_antidegeneration_enabled: bool = False,
        adaptive_window_frames: int = 12,
        adaptive_repeat_frame_ratio_threshold: float = 0.5,
        adaptive_consecutive_repeat_frames_threshold: int = 4,
        adaptive_audio_temperature: float = 0.7,
        adaptive_audio_top_p: float = 0.85,
        adaptive_audio_top_k: int = 10,
        adaptive_audio_repetition_penalty: float = 1.5,
        adaptive_force_stop_consecutive_repeat_frames: int = 10,
        repetition_aware_logit_shaping_enabled: bool = False,
        repetition_aware_window_tokens: int = 32,
        repetition_aware_frequency_penalty: float = 0.15,
        repetition_aware_consecutive_penalty: float = 0.5,
        ...
        entropy_aware_top_p: float = 0.8,
        entropy_aware_top_k: int = 10,
        entropy_aware_hysteresis_enabled: bool = False,
        entropy_aware_enter_high_steps: int = 2,
        entropy_aware_exit_low_steps: int = 2,
        entropy_triggered_short_horizon_branching_enabled: bool = False,
        entropy_triggered_short_horizon_branching_threshold: float = 0.6,
        entropy_triggered_short_horizon_branching_num_candidates: int = 4,
        entropy_triggered_short_horizon_branching_repeat_frame_penalty: float = 0.75,
        ...
        effective_text_do_sample = do_sample if text_do_sample is None else bool(text_do_sample)
        effective_audio_do_sample = do_sample if audio_do_sample is None else bool(audio_do_sample)
        entropy_aware_active_state = torch.zeros(batch_size, dtype=torch.bool, device=input_ids.device)
        entropy_aware_high_counts = torch.zeros(batch_size, dtype=torch.long, device=input_ids.device)
        entropy_aware_low_counts = torch.zeros(batch_size, dtype=torch.long, device=input_ids.device)
        ...
        for step_index in range(max_new_frames):
            ...
            active_generated_audio_history = (
                None if generated_audio_history is None else generated_audio_history[:, :, :effective_nq]
            )
            ...
            next_text_tokens = self._sample_next_assistant_text_token(
                logits=text_logits,
                do_sample=effective_text_do_sample,
            ...
            adaptive_row_mask = torch.zeros(batch_size, dtype=torch.bool, device=input_ids.device)
            repeat_frame_ratio = torch.zeros(batch_size, dtype=torch.float32, device=input_ids.device)
            consecutive_repeat_frames = torch.zeros(batch_size, dtype=torch.long, device=input_ids.device)
            if adaptive_antidegeneration_enabled and active_generated_audio_history is not None:
                repeat_frame_ratio, consecutive_repeat_frames = self._compute_recent_frame_repeat_stats(
                    frame_history=active_generated_audio_history,
                    window_frames=adaptive_window_frames,
                )
                adaptive_row_mask = (
                    should_continue
                    & ~finished
                    & (
                        (repeat_frame_ratio >= float(adaptive_repeat_frame_ratio_threshold))
                        | (consecutive_repeat_frames >= int(adaptive_consecutive_repeat_frames_threshold))
                    )
                )

            next_frame_tokens = []
            current_local_input = self.transformer.wte(next_text_tokens).to(dtype=local_dtype)
            step_entropy_aware_enabled = entropy_aware_decoding_enabled
            if (
                entropy_aware_decoding_enabled
                and entropy_aware_hysteresis_enabled
                and effective_audio_do_sample
            ):
                first_history = None if active_generated_audio_history is None else active_generated_audio_history[:, :, 0]
                first_local_inputs = torch.cat([local_inputs_embeds, current_local_input.unsqueeze(1)], dim=1)
                first_local_hidden = self._decode_local_last_hidden_state(first_local_inputs)
                ...
                _, first_channel_probs = self._prepare_sampling_distribution(
                    logits=first_channel_logits,
                    do_sample=effective_audio_do_sample,
                    temperature=audio_temperature,
                    top_k=audio_top_k,
                    top_p=audio_top_p,
                    previous_token_ids=first_history,
                    ...
                    repetition_aware_frequency_penalty=repetition_aware_frequency_penalty,
                    repetition_aware_consecutive_penalty=repetition_aware_consecutive_penalty,
                    entropy_aware_decoding_enabled=False,
                    entropy_aware_low_entropy=entropy_aware_low_entropy,
                    entropy_aware_high_entropy=entropy_aware_high_entropy,
                    entropy_aware_temperature=entropy_aware_temperature,
                    entropy_aware_top_p=entropy_aware_top_p,
                ...
                entropy_values = self._normalized_distribution_entropy(first_channel_probs)
                (
                    entropy_aware_active_state,
                    entropy_aware_high_counts,
                    entropy_aware_low_counts,
                ) = self._update_entropy_aware_state(
                    entropy_values=entropy_values,
                    active_state=entropy_aware_active_state,
                    high_counts=entropy_aware_high_counts,
                    low_counts=entropy_aware_low_counts,
                    low_entropy=entropy_aware_low_entropy,
                    high_entropy=entropy_aware_high_entropy,
                    enter_high_steps=entropy_aware_enter_high_steps,
                    exit_low_steps=entropy_aware_exit_low_steps,
                )
                step_entropy_aware_enabled = bool(entropy_aware_active_state.any().item())
            branch_frame_prefix = self._select_entropy_triggered_branch_frame(
                local_inputs_embeds=local_inputs_embeds,
                current_local_input=current_local_input,
                effective_nq=effective_nq,
                active_generated_audio_history=active_generated_audio_history,
                do_sample=effective_audio_do_sample,
                audio_temperature=audio_temperature,
                audio_top_p=audio_top_p,
                audio_top_k=audio_top_k,
                ...
                repetition_aware_frequency_penalty=repetition_aware_frequency_penalty,
                repetition_aware_consecutive_penalty=repetition_aware_consecutive_penalty,
                entropy_aware_decoding_enabled=step_entropy_aware_enabled,
                entropy_aware_low_entropy=entropy_aware_low_entropy,
                entropy_aware_high_entropy=entropy_aware_high_entropy,
                entropy_aware_temperature=entropy_aware_temperature,
                entropy_aware_top_p=entropy_aware_top_p,
                entropy_aware_top_k=entropy_aware_top_k,
                threshold=entropy_triggered_short_horizon_branching_threshold,
                num_candidates=entropy_triggered_short_horizon_branching_num_candidates,
                repeat_frame_penalty=entropy_triggered_short_horizon_branching_repeat_frame_penalty,
                local_dtype=local_dtype,
            ) if entropy_triggered_short_horizon_branching_enabled and batch_size == 1 else None
            if branch_frame_prefix is None:
                for channel_index in range(effective_nq):
                    ...
                    channel_token = self._sample_next_token(
                        ...
                        top_p=audio_top_p,
                        previous_token_ids=(
                            None if active_generated_audio_history is None else active_generated_audio_history[:, :, channel_index]
                        ),
                        repetition_penalty=audio_repetition_penalty,
                        repetition_aware_logit_shaping_enabled=repetition_aware_logit_shaping_enabled,
                        repetition_aware_window_tokens=repetition_aware_window_tokens,
                    ...
                    if adaptive_row_mask.any():
                        adaptive_channel_token = self._sample_next_token(
                            logits=channel_logits,
                            do_sample=effective_audio_do_sample,
                            temperature=adaptive_audio_temperature,
                            top_k=adaptive_audio_top_k,
                            top_p=adaptive_audio_top_p,
                            previous_token_ids=(
                                None
                                if active_generated_audio_history is None
                                else active_generated_audio_history[:, :, channel_index]
                            ),
                            repetition_penalty=adaptive_audio_repetition_penalty,
                            repetition_aware_logit_shaping_enabled=repetition_aware_logit_shaping_enabled,
                            repetition_aware_window_tokens=repetition_aware_window_tokens,
                            repetition_aware_frequency_penalty=repetition_aware_frequency_penalty,
                            repetition_aware_consecutive_penalty=repetition_aware_consecutive_penalty,
                        ...
                        channel_token = self._merge_rowwise_sampled_tokens(
                            base_tokens=channel_token,
                            override_tokens=adaptive_channel_token,
                            override_mask=adaptive_row_mask,
                        )
                ...
                next_frame_prefix = branch_frame_prefix
            ...
            if adaptive_antidegeneration_enabled and active_generated_audio_history is not None:
                repeated_with_previous = next_frame[:, :effective_nq].eq(active_generated_audio_history[:, -1, :]).all(dim=-1)
                forced_stop_mask = (
                    should_continue
                    & repeated_with_previous
                    & (
                        consecutive_repeat_frames + 1
                        >= int(adaptive_force_stop_consecutive_repeat_frames)
                    )
                )
                if forced_stop_mask.any():
                    should_continue = should_continue & ~forced_stop_mask
                    finished = finished | forced_stop_mask
            ...
            yield {
                ...
                "adaptive_row_mask": adaptive_row_mask.detach().clone(),
            }
